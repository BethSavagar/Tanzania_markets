...
# survey_count %>% filter(n<20) %>% kable()
...
# write.csv(vilmrkt_srvy, file = "vilmrkt_srvy.csv", row.names = F)



wdvilmrkt_srvy <- mrktc_details %>% 
  group_by(ward.code,village,market.name) %>% 
  count() %>% 
  # View() %>% 
  # group_by(village) %>% 
  # count() %>% 
...
mrktc_mrktvisits_region <- mrktc_clean %>%
  select("district.region.name", "other.market.visited", "number.markets.visited","trader") %>%
  mutate(number.markets.visited = ifelse(`other.market.visited`==0, 0,`number.markets.visited`),
         # trader = ifelse(trader==1,"yes", ifelse(trader==0,"no",NA))
         )

ggplot(mrktc_mrktvisits_region, aes(factor(number.markets.visited), group = trader, fill= factor(trader)))+
  geom_bar(stat="count", position = "dodge")+
  labs(x="number of other markets visited",
       fill = "trader")+
