...
## 2. How many times in a month do you buy or sell sheep or goats in this market?
...
buysell_mnth <-  mrktactivities_clean %>%
  group_by(trader, `12.buy.sell.per.month`) %>%
  count() %>%
  mutate(prop = n/3310) %>% 
  filter(!`12.buy.sell.per.month` %in% c(5000,10000,20000,230000))

ggplot(buysell_mnth, aes(x=factor(`12.buy.sell.per.month`), y=n, group = trader, fill = as.factor(trader)))+
  geom_col(aes(fill = as.factor(trader)), position = "dodge")+
  labs(x="How many times buy/sell at this market per month",
       y="count",
       fill = "Trader")+
  scale_fill_discrete(labels = c("N","Y"))+
  annotate("label", x = 3, y = 1500, label = paste0("total=",total_obs))+
  theme_bw(base_size = 14) # +
  # theme(axis.text.x = element_text(angle = -20, hjust = 0, vjust = 1))

#### NEED TO ADD REGIONAL ANALYSIS HERE ####
...
selling <- selling %>% 
  ...
  mutate(`2.sell.placetype.name` = ifelse(`2.sell.placetype.name`=="Other (give details)", "Other", `2.sell.placetype.name`))

sellers <- ggplot(selling, aes(`2.sell.placetype.name`))+
  geom_bar()+
  geom_text(stat='count', aes(label=..count..), vjust=-1)+
  annotate("label", x = 1, y = 1500, label = paste0("total=",nrow(selling)))+
  labs(title = "Seller",y="Count")+
  theme_bw(base_size=14)
...
traders <- ggplot(trading, aes(`2.sell.placetype.name`))+
  geom_bar()+
  geom_text(stat='count', aes(label=..count..), vjust=-1)+
  annotate("label", x = 1, y = 1500, label = paste0("total=",nrow(trading)))+
  labs(title = "Trader",y="Count")+
  theme_bw(base_size=14)
...
middlers <- ggplot(middle, aes(`2.sell.placetype.name`))+
  geom_bar()+
  geom_text(stat='count', aes(label=..count..), vjust=-1)+
  annotate("label", x = 1, y = 1500, label = paste0("total=",nrow(middle)))+
  labs(title = "Middlemen",y="Count")+
  theme_bw(base_size=14)

middlers

### 
buyer <- mrktactivities_clean %>%
  filter(buyer =="1") %>%
  select(`fid.see.mrktc.generalinfo.`,
         `unique.row.id.see.mrktc.generalinfo.`,
         `unique.row.id`,
...
buyers <- ggplot(buyer, aes(`2.sell.placetype.name`))+
  geom_bar()+
  geom_text(stat='count', aes(label=..count..), vjust=-1)+
  annotate("label", x = 1, y = 1500, label = paste0("total=",nrow(buyer)))+
  labs(title = "Buyer",y="Count")+
  theme_bw(base_size=14)

buyers

ggarrange(sellers, traders, middlers, buyers, ncol = 2, nrow = 2)
```
